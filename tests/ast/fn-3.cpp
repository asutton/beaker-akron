...
main(int argc, char* argv[])
{
  ...
  // Some declarations.
  decl* vars[] {
    &cb.make_parm_decl(cb.get_name("a"), rb),
    &cb.make_parm_decl(cb.get_name("b"), ri32),
    &cb.make_parm_decl(cb.get_name("c"), ri1024),
    &cb.make_parm_decl(cb.get_name("r"), i32)
  };

  // Passes a couple objects by reference, returns a copy.
  typed_decl* d1;
  { // f1 : (bool&, int32&) -> i32
